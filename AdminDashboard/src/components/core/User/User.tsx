...
const UserOrder = ({id}) => {
  ...
  useEffect(() => { 
    ...
    const getOrders = async () => {
      ...
      try {
        const response = await axios.get(`${BASE_URL}/api/v1/payment/getUserOrders/${id}`);
  ...
  return (
    <div>
      <button
        className="text-richblack-25 mx-auto p-6 bg-gray-100 rounded-lg shadow-md"
      ...
      {
       !isModalOpen && (
          <div className="text-center text-richblack-5 mt-4">No orders found</div>)
      }
      ...
      {isModalOpen && (
         ...
         <div className="bg-black p-6 rounded-lg shadow-md max-h-full overflow-y-auto w-[60%]">
           ...
           {
              orders.length === 0 && (
                <div className="text-center text-richblack-5 mt-4">No orders found ...........
                <LoaderIcon 
